...
require_relative 'treasure_kind.rb'
  ...
  monsters.each do |monsters|
    ...
    if((monsters.BadConsequence.some_visible_treasures == 0) && (monsters.BadConsequence.some_hidden_treasures == 0)) 
      if(monsters.BadConsequence.levels > 0)
        puts monsters.to_s
      end
    end
  end
  ...
  puts "\n\n*******************Mostrar todos los monstruos que impliquen la pérdida de un determinado tipo de tesoros visibles y/o ocultos"
  
        monsters.each do |monsters|
      
      if (!(monsters.BadConsequence.some_specific_visible_treasures.empty?) || !(monsters.BadConsequence.some_specific_hidden_treasures.empty?))
        puts monsters.to_s
      end
